  ...
  const projects = [
    {
      ...
      featured: true,
      challenges: 'Synchronizing layout rendering between co-engineered layers while maintaining custom CSS scoping across sections.',
      solutions: 'Integrated atomic Tailwind utility chains alongside modular section overrides, reducing render blocking by 40%.'
    },
    {
      id: 'jolly-shaifs',
      title: 'Jolly Shaifs',
      ...
      category: 'Web App',
      featured: false,
      challenges: 'Configuring high-momentum scroll indicators and layout adapters for ultra-widescreen rendering.',
      solutions: 'Utilized Framer Motion spring configurations and adaptive CSS variables for viewport-based responsive transitions.'
    },
    {
      id: 'mavel-tech',
      title: 'Mavel Tech',
      ...
      featured: false,
      challenges: 'Presenting multi-layer technical assets and documentation in a fast-loading, highly structured dashboard view.',
      solutions: 'Built dynamic navigation tree lists utilizing static asset imports and memoized routing tabs to eliminate re-rendering lag.'
    },
    {
      id: 'rozerr',
      title: 'Rozerr',
      ...
      category: 'Web App',
      featured: false,
      challenges: 'Maintaining a clean state machine for dynamic layouts and interactive design tokens.',
      solutions: 'Created centralized theme context and lightweight component abstractions to handle style prop passes efficiently.'
    },
    {
      id: 'blinkstore',
      title: 'BlinkStore',
      ...
      featured: false,
      challenges: 'Integrating Solana wallet adapter pipelines and mapping dynamic on-chain action payloads to Twitter-compatible blinks.',
      solutions: 'Utilized Solana Actions SDK along with Rust transaction signing helpers to execute lightning-fast micro-payments directly from URLs.'
    },
    {
      id: 'pumptroll',
      title: 'PumpTroll',
      ...
      category: 'Web3 App',
      featured: false,
      challenges: 'Structuring state pipelines to track wallet connectivity changes and polling real-time APIs without memory leaks.',
      solutions: 'Configured clean React hooks for state subscriptions and local storage synchronization, coupled with auto-cleanup timeouts.'
    },
    {
      id: 'chaincrafter',
      title: 'ChainCrafter',
      ...
      category: 'Dev Tool',
      featured: false,
      challenges: 'Building a smart contract visualizer that handles multi-protocol interaction states dynamically.',
      solutions: 'Designed modular JSON payload parsers that map ABI methods directly to customizable react form elements.'
    },
    {
      id: 'verified-hearts',
      ...
      featured: false,
      challenges: 'Ensuring extreme accessibility (a11y) and fast loading times under low-bandwidth network environments.',
      solutions: 'Implemented strict semantic HTML layout structures, compressed lightweight media formats, and minimized client-side JS bundle sizes.'
    },
    {
      id: 'estattery',
      ...
      featured: false,
      challenges: 'Designing complex query builders for multi-parameter real estate property searches with zero lag.',
      solutions: 'Configured index-backed MongoDB query schemas, debounced search updates, and cached property results for recurring parameters.'
    }
  ];
  ...
  const techStack = [
    {
      ...
      items: [
        { name: 'HTML5', icon: Globe, tooltip: 'Semantic document structure, SEO tags layout, and strict Web Accessibility (a11y) compliance.' },
        { name: 'CSS3 / SASS', icon: Palette, tooltip: 'Advanced keyframe animations, responsive grid engines, custom transitions, and CSS custom properties (variables).' },
        { name: 'JavaScript (ES6+)', icon: Zap, tooltip: 'Asynchronous event architectures, DOM reconciliation, JSON data mapping, and local storage caches.' },
        { name: 'TypeScript', icon: FileCode, tooltip: 'Strict build-time type verification, custom code interfaces, and structured application pipelines.' },
        { name: 'React', icon: Atom, tooltip: 'Virtual DOM, state rendering lifecycle, component composition, hooks, and context management.' },
        { name: 'Vite', icon: Rocket, tooltip: 'Lightning-fast module replacement (HMR), tree-shaking logic, and optimized bundler pipelines.' },
        { name: 'Tailwind CSS', icon: Wind, tooltip: 'Atomic design tokens, utility utility classes, responsive grid overrides, and rapid UI development.' },
        { name: 'Bootstrap', icon: Layers, tooltip: 'Legacy library integration, boilerplate layout variables, and quick prototype design frameworks.' },
      ]
    },
    {
      category: 'Backend & Logic',
      items: [
        { name: 'Node.js', icon: Server, tooltip: 'High-concurrency Javascript server runtime, event-driven I/O, and server logic controllers.' },
        { name: 'Express.js', icon: Cpu, tooltip: 'Modular routes, API request mapping middleware, error handlers, and security parameters.' },
        { name: 'REST APIs', icon: Network, tooltip: 'Standardized query parameters, content pagination, rate limit protocols, and clean routing rules.' },
        { name: 'MERN Architecture', icon: Blocks, tooltip: 'Cohesive database validation schemas, route controllers, and full-stack component synchronization.' },
      ]
    },
    {
      category: 'Systems & Web3 Blockchain',
      items: [
        { name: 'Rust', icon: Shield, tooltip: 'High-performance compile-time memory validation, smart-contracts, and WASM bindings.' },
        { name: 'Solana Web3', icon: Boxes, tooltip: 'On-chain program logs, RPC endpoint triggers, wallet balance states, and transaction signing.' },
        { name: 'Blinks / Actions', icon: Zap, tooltip: 'Twitter action pipelines, payment URLs, and standard Web3 wallet transaction bindings.' },
      ]
    }
  ];
        ...
        <section id="techstack" className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 space-y-8">
          <div className="text-center space-y-3">
            ...
            <p className="text-sm text-[var(--text-body)] max-w-xl mx-auto font-mono">
              Hover over stack icons to view specific architectural applications.
            </p>
          </div>
                      ...
                      return (
                        <Tooltip key={tIdx}>
                          <TooltipTrigger asChild>
                            <div className="p-3 rounded-xl bg-[var(--bg-primary)] border border-[var(--border-color)] hover:border-[var(--accent)] transition-all duration-200 group flex items-center gap-2.5 cursor-pointer">
          ...
          <div className="flex flex-col md:flex-row md:items-end justify-between gap-4 border-b border-[var(--border-color)] pb-6">
            <div>
              ...
              <p className="text-sm text-[var(--text-body)] font-mono mt-1">
                Click any project card to review technical case studies, challenges, and architecture.
              </p>
            </div>
            ...
            {projects.map((project) => (
              <Dialog key={project.id}>
                ...
                >
                  <DialogTrigger asChild>
                    <div className="cursor-pointer flex-1">
                      <div className="h-48 relative overflow-hidden bg-slate-900 border-b border-[var(--border-color)]">
                        ...
                        {/* Interactive Case Study Hover Overlay */}
                        <div className="absolute inset-0 flex items-center justify-center bg-slate-950/60 opacity-0 group-hover:opacity-100 transition-opacity duration-300 backdrop-blur-sm z-20">
                          <span className="text-xs font-mono font-bold text-slate-950 bg-[var(--accent)] px-4 py-2 rounded-xl shadow-lg border border-[var(--accent)]">
                            [ VIEW TECH SPECS ]
                          </span>
                        </div>
                      </div>
                  ...
                  <div className="p-6 pt-0 border-t border-[var(--border-color)] mt-4 flex items-center justify-between gap-2 z-30">
                    <a
                      href={project.url}
                      target="_blank"
                ...
                {/* Case Study dialog content details */}
                <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto w-[92vw]">
                  <DialogHeader className="border-b border-[var(--border-color)] pb-4">
                    <DialogTitle className="text-xl font-pixel text-[var(--text-heading)] mb-1">
                      {project.title}
                    </DialogTitle>
                    <DialogDescription className="text-xs font-mono text-[var(--accent)]">
                      {project.tagline}
                    </DialogDescription>
                  </DialogHeader>

                  <div className="py-4 space-y-6 text-xs sm:text-sm font-mono">
                    {/* Category and Tags */}
                    <div className="flex flex-wrap items-center justify-between gap-2 bg-[var(--bg-primary)] p-3 rounded-xl border border-[var(--border-color)]">
                      <span className="text-[11px] text-[var(--text-muted)] font-bold">
                        CATEGORY: <span className="text-[var(--accent)]">{project.category}</span>
                      </span>
                      <div className="flex flex-wrap gap-1.5">
                        {project.tags.map((tag, idx) => (
                          <span key={idx} className="text-[10px] px-2 py-0.5 rounded bg-[var(--bg-card)] text-[var(--text-heading)] border border-[var(--border-color)]">
                            #{tag}
                          </span>
                        ))}
                      </div>
                    ...
                    <div className="space-y-1.5">
                      <h4 className="text-xs font-bold text-[var(--accent)] uppercase tracking-wider flex items-center gap-1.5">
                        <Terminal className="w-3.5 h-3.5" /> PROJECT OVERVIEW
                      </h4>
                      <p className="font-sans text-xs text-[var(--text-body)] leading-relaxed p-4 rounded-xl bg-[var(--bg-primary)] border border-[var(--border-color)]">
                        {project.description}
                      </p>
                    </div>
                    ...
                    <div className="space-y-1.5">
                      <h4 className="text-xs font-bold text-[var(--accent)] uppercase tracking-wider flex items-center gap-1.5">
                        <Cpu className="w-3.5 h-3.5 text-amber-400 animate-pulse-slow" /> TECHNICAL CHALLENGES
                      </h4>
                      <p className="font-sans text-xs text-[var(--text-body)] leading-relaxed p-4 rounded-xl bg-[var(--bg-primary)] border border-[var(--border-color)]">
                        {project.challenges}
                      </p>
                    ...
                    <div className="space-y-1.5">
                      <h4 className="text-xs font-bold text-[var(--accent)] uppercase tracking-wider flex items-center gap-1.5">
                        <Zap className="w-3.5 h-3.5 text-emerald-400" /> IMPLEMENTED SOLUTIONS
                      </h4>
                      <p className="font-sans text-xs text-[var(--text-body)] leading-relaxed p-4 rounded-xl bg-[var(--bg-primary)] border border-[var(--border-color)]">
                        {project.solutions}
                      </p>
                  ...
                  {/* Footer Buttons */}
                  <div className="border-t border-[var(--border-color)] pt-4 flex flex-wrap items-center justify-between gap-3">
                    <div className="flex items-center gap-2">
                      <a
                        href={project.url}
                        target="_blank"
                        rel="noopener noreferrer"
                        className="py-2.5 px-4 rounded-xl bg-[var(--accent)] text-slate-950 hover:opacity-90 text-xs font-bold flex items-center gap-1.5 transition-all"
                      >
                        <span>LIVE DEMO</span>
                        <ExternalLink className="w-3.5 h-3.5" />
                      </a>
                      ...
                      {project.mirrorUrl && (
                        <a
                          ...
                          target="_blank"
                          rel="noopener noreferrer"
                          className="py-2.5 px-4 rounded-xl bg-[var(--bg-primary)] text-[var(--text-body)] hover:text-[var(--text-heading)] border border-[var(--border-color)] text-xs font-semibold flex items-center gap-1.5 transition-all"
                        >
                    ...
                    <button
                      onClick={() => handleCopyLink(project.url, project.id)}
                      className="py-2.5 px-4 rounded-xl bg-[var(--bg-primary)] text-[var(--text-body)] hover:text-[var(--accent)] border border-[var(--border-color)] text-xs font-semibold flex items-center gap-2 transition-all"
                    >
                      {copiedLink === project.id ? (
                        <>
