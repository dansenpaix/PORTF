...
const AccordionTrigger = React.forwardRef(({ className, children, ...props }, ref) => (
  <AccordionPrimitive.Header className="flex">
    <AccordionPrimitive.Trigger
      ref={ref}
      className={cn(
        "flex flex-1 items-center justify-between py-4 font-mono text-xs font-semibold text-[var(--accent)] hover:underline transition-all [&[data-state=open]>svg]:rotate-180",
        className
      )}
      {...props}
    >
...
const AccordionContent = React.forwardRef(({ className, children, ...props }, ref) => (
  <AccordionPrimitive.Content
    ...
    {...props}
  >
    <div className={cn("pb-4 pt-0 font-sans text-[var(--text-body)] leading-relaxed", className)}>{children}</div>
  </AccordionPrimitive.Content>
));
